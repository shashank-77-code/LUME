...
function StatsStrip({ result }: { result: MigrationResponse | null }) {
  ...
  return <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{metrics.map((metric, index) => { const Icon = metric.icon; return <motion.div animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-line bg-surface-panel px-4 py-4 shadow-panel transition-[transform,border-color,box-shadow] duration-500 hover:-translate-y-0.5 hover:border-brand-orange/25 hover:shadow-glow-primary sm:px-5" initial={reduceMotion ? false : { opacity: 0, y: 14 }} key={metric.label} transition={{ delay: index * 0.07, duration: 0.5, ease: 'easeOut' }}><div className="flex items-center justify-between gap-2"><span className="text-[0.6875rem] font-semibold text-ink-muted">{metric.label}</span><Icon className={metric.color} size={16} /></div><div className="mt-3 flex items-end justify-between gap-2"><span className="font-display text-2xl font-extrabold tracking-[-0.04em] text-ink-primary">{metric.value}</span><span className="hidden text-right text-[0.6rem] text-ink-subtle sm:block">{metric.note}</span></div></motion.div>; })}</div>;
}
