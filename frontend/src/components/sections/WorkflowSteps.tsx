    ...
    <section aria-labelledby="workspace-title" className="min-h-screen bg-[#07080c] text-ink-primary" id="workflow">
      <div className="mx-auto flex min-h-screen max-w-[112rem] lg:grid lg:grid-cols-[15.5rem_minmax(0,1fr)]">
        ...
        <main className="min-w-0 px-4 pb-10 pt-4 sm:px-6 lg:px-8 lg:pt-7">
          <motion.div animate={{ opacity: 1, y: 0 }} className="mb-7 flex flex-col items-start justify-between gap-4 sm:flex-row" initial={reduceMotion ? false : { opacity: 0, y: 16 }} transition={{ duration: 0.5, ease: 'easeOut' }}>
            <div>
...
function WorkspaceSidebar({ onNewMigration, onUpload }: { onNewMigration: () => void; onUpload: () => void }) {
  ...
  return (
    <motion.aside animate={{ opacity: 1, x: 0 }} className="hidden border-r border-line bg-[#090a0f] px-4 py-6 lg:flex lg:flex-col" initial={reduceMotion ? false : { opacity: 0, x: -18 }} transition={{ duration: 0.5, ease: 'easeOut' }}>
      <Link aria-label="LUME home" className="group flex items-center gap-2.5 rounded-lg px-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-orange" to="/"><BrandMark className="h-8 w-8 transition-transform duration-300 group-hover:rotate-45" /><span className="font-display text-lg font-extrabold tracking-[0.14em]">LUME</span></Link>
...
function StatsStrip({ result }: { result: MigrationResponse | null }) {
  ...
  return <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{metrics.map((metric, index) => { const Icon = metric.icon; return <motion.div className="rounded-2xl border border-line bg-surface-panel px-4 py-4 shadow-panel transition-[transform,border-color,box-shadow] duration-500 hover:-translate-y-0.5 hover:border-brand-orange/25 hover:shadow-glow-primary sm:px-5" initial={reduceMotion ? false : { opacity: 0, y: 14 }} key={metric.label} transition={{ delay: index * 0.07, duration: 0.5, ease: 'easeOut' }} viewport={{ once: true, amount: 0.25 }} whileInView={{ opacity: 1, y: 0 }}><div className="flex items-center justify-between gap-2"><span className="text-[0.6875rem] font-semibold text-ink-muted">{metric.label}</span><Icon className={metric.color} size={16} /></div><div className="mt-3 flex items-end justify-between gap-2"><span className="font-display text-2xl font-extrabold tracking-[-0.04em] text-ink-primary">{metric.value}</span><span className="hidden text-right text-[0.6rem] text-ink-subtle sm:block">{metric.note}</span></div></motion.div>; })}</div>;
}
...
function CodeEditor({ code, tone }: { code: string; tone: 'legacy' | 'modern' }) {
  ...
  return <div className="workspace-scrollbar h-[32rem] overflow-auto scroll-smooth bg-[#05060a] py-4 font-mono text-[0.7rem] leading-6 sm:h-[38rem] 2xl:h-[42rem]"><div className="min-w-max"><div className={`min-w-max ${tone === 'modern' ? 'text-ink-muted' : 'text-ink-subtle'}`}>{lines.map((line, index) => <div className="flex min-w-max" key={`${line}-${index}`}><span className="sticky left-0 z-10 inline-block w-[2.75rem] shrink-0 select-none border-r border-line bg-[#05060a] px-3 text-right text-ink-subtle/50 shadow-[0.35rem_0_0.8rem_rgb(0_0_0_/_20%)]">{index + 1}</span><code className="px-5">{highlightCode(line)}</code></div>)}</div></div></div>;
}

function EmptyEditor({ status }: { status: WorkspaceStatus }) {
  return <div className="flex h-[32rem] flex-col items-center justify-center px-8 text-center sm:h-[38rem] 2xl:h-[42rem]"><div className="mb-4 grid h-12 w-12 place-items-center rounded-2xl border border-line bg-surface-elevated text-ink-subtle"><Code2 size={22} /></div><p className="text-sm font-extrabold text-ink-primary">{status === 'error' ? 'Scan could not be completed' : 'Editor ready for your migration'}</p><p className="mt-2 max-w-sm text-xs leading-5 text-ink-muted">Start a scan from the source panel and LUME will show the transformed output with line-level context here.</p><div className="mt-5 flex items-center gap-2 font-mono text-[0.6rem] text-ink-subtle"><span className="h-1.5 w-1.5 rounded-full bg-ink-subtle/60" />No active file</div></div>;
}
...
function FindingsList({ activeFinding, allFindings, explainFinding, result, setActiveFile }: { activeFinding: Finding | null; allFindings: Finding[]; explainFinding: (finding: Finding) => void; result: MigrationResponse; setActiveFile: (index: number) => void }) {
  const reduceMotion = useReducedMotion();
  return <div><div className="mb-3 flex items-center justify-between"><span className="text-xs font-extrabold text-ink-primary">{allFindings.length} migration findings</span><span className="font-mono text-[0.58rem] uppercase tracking-[0.1em] text-ink-subtle">Select to inspect</span></div><div className="workspace-scrollbar h-[22rem] space-y-2 overflow-y-auto scroll-smooth pr-2">{allFindings.length ? allFindings.map((finding, index) => { const active = activeFinding?.id === finding.id; return <motion.div animate={{ opacity: 1, y: 0 }} initial={reduceMotion ? false : { opacity: 0, y: 12 }} key={finding.id} transition={{ delay: index * 0.06, duration: 0.45, ease: 'easeOut' }} viewport={{ once: true, amount: 0.2 }} whileInView={{ opacity: 1, y: 0 }}><button className={`w-full rounded-xl border p-3 text-left transition-[transform,border-color,background-color,box-shadow] duration-300 hover:-translate-y-0.5 hover:shadow-panel ${active ? 'border-brand-orange/60 bg-brand-orange/[0.07]' : 'border-line bg-surface-base/45 hover:border-brand-orange/40 hover:bg-surface-elevated'}`} onClick={() => { const fileIndex = result.files.findIndex((file) => file.file_path === finding.file_path); if (fileIndex >= 0) setActiveFile(fileIndex); explainFinding(finding); }} type="button"><div className="flex items-start justify-between gap-3"><span className="font-mono text-[0.6rem] text-brand-orange">{finding.file_path}:{finding.line_number}</span><SeverityBadge severity={finding.severity} /></div><p className="mt-1 text-xs font-extrabold text-ink-primary">{finding.rule_name}</p><p className="mt-1 truncate font-mono text-[0.6rem] text-ink-muted">{finding.code_snippet}</p></button></motion.div>; }) : <div className="rounded-xl border border-status/25 bg-status/5 p-4 text-xs text-status">No migration findings detected in the scanned files.</div>}</div></div>;
}
...
function FindingDetail({ activeFinding }: { activeFinding: Finding | null }) {
  ...
  return <div className="rounded-xl border border-line bg-surface-base/55 p-4"><div className="flex items-start justify-between gap-3"><div><span className="font-mono text-[0.58rem] uppercase tracking-[0.14em] text-brand-orange">Finding detail</span><h3 className="mt-1 text-sm font-extrabold text-ink-primary">{activeFinding.rule_name}</h3></div><SeverityBadge severity={activeFinding.severity} /></div><div className="mt-4 space-y-3 text-xs"><div><p className="font-mono text-[0.57rem] uppercase tracking-[0.12em] text-ink-subtle">Legacy API</p><code className="workspace-scrollbar mt-1 block max-h-32 overflow-auto rounded-lg bg-[#05060a] p-2.5 font-mono text-[0.67rem] text-brand-orange">{activeFinding.code_snippet}</code></div><div><p className="font-mono text-[0.57rem] uppercase tracking-[0.12em] text-ink-subtle">Suggested migration</p><code className="workspace-scrollbar mt-1 block max-h-32 overflow-auto rounded-lg bg-[#05060a] p-2.5 font-mono text-[0.67rem] text-status">{activeFinding.suggested_replacement}</code></div><p className="leading-5 text-ink-muted">{activeFinding.why_changed}</p><p className="leading-5 text-ink-muted">{activeFinding.migration_advice}</p></div></div>;
}

function AiExplanationPanel({ activeFinding, explanation }: { activeFinding: Finding | null; explanation: Explanation | null }) {
  const reduceMotion = useReducedMotion();
  return <motion.div animate={{ opacity: 1, x: 0 }} className="rounded-xl border border-brand-orange/20 bg-brand-orange/[0.035] p-4" id="explanation-panel" initial={reduceMotion ? false : { opacity: 0, x: 24 }} transition={{ duration: 0.5, ease: 'easeOut' }}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg border border-brand-orange/30 bg-brand-orange/10 text-brand-orange"><Sparkles size={14} /></span><div><p className="text-xs font-extrabold text-ink-primary">AI explanation</p><p className="text-[0.62rem] text-ink-muted">Context for your selected migration finding</p></div></div><span className="rounded-md border border-line px-2 py-1 font-mono text-[0.56rem] uppercase text-ink-subtle">Optional</span></div><div className="workspace-scrollbar mt-4 max-h-[14rem] overflow-y-auto scroll-smooth pr-2">{activeFinding && explanation ? <div className="grid gap-4 md:grid-cols-[1fr_0.8fr]"><div><p className="text-xs font-bold text-ink-primary">{explanation.summary}</p><p className="mt-2 text-xs leading-5 text-ink-muted">{explanation.rationale}</p></div><div className="rounded-lg border border-line bg-surface-base/45 p-3"><p className="font-mono text-[0.56rem] uppercase tracking-[0.12em] text-ink-subtle">Migration steps</p><ul className="mt-2 space-y-1.5 text-xs text-ink-muted">{explanation.migration_steps.map((step) => <li className="flex gap-2" key={step}><Check className="mt-0.5 shrink-0 text-status" size={13} />{step}</li>)}</ul></div></div> : <p className="text-xs leading-5 text-ink-muted">{activeFinding ? 'Generating an explanation for this finding...' : 'Select a finding to generate a plain-language explanation and recommended next steps.'}</p>}</div></motion.div>;
}
...
function MigrationPreview({ currentFile, copyCode, copied, result, setActiveFile }: { currentFile?: ScannedFile; copyCode: () => void; copied: boolean; result: MigrationResponse; setActiveFile: (index: number) => void }) {
  ...
  const reduceMotion = useReducedMotion();
  return <motion.div animate={{ opacity: 1, y: 0 }} className="space-y-3 pt-4" initial={reduceMotion ? false : { opacity: 0, y: 14 }} transition={{ duration: 0.5, ease: 'easeOut' }}><FilePicker result={result} setActiveFile={setActiveFile} /><div className="flex items-center justify-between"><div className="flex items-center gap-4 font-mono text-[0.58rem] uppercase tracking-[0.12em]"><span className="text-brand-orange">Legacy source</span><span className="text-status">Migrated output</span></div><button className="flex items-center gap-1.5 text-xs font-bold text-ink-muted transition-transform hover:-translate-y-0.5 hover:text-brand-orange" onClick={copyCode} type="button">{copied ? <Check size={14} /> : <Copy size={14} />}{copied ? 'Copied' : 'Copy output'}</button></div><div className="workspace-scrollbar h-[28rem] overflow-auto scroll-smooth rounded-xl border border-line bg-line"><div className="grid min-w-[42rem] gap-px md:grid-cols-2"><CodePaneLines lines={originalLines} counterpart={transformedLines} tone="legacy" /><CodePaneLines lines={transformedLines} counterpart={originalLines} tone="modern" /></div></div><details className="rounded-xl border border-line bg-surface-base/40 p-3"><summary className="flex cursor-pointer items-center gap-2 text-xs font-extrabold text-ink-primary"><ChevronDown size={14} />Unified diff</summary><pre className="workspace-scrollbar mt-3 max-h-48 overflow-auto scroll-smooth font-mono text-[0.63rem] leading-5 text-ink-muted">{currentFile?.diff_text || 'No changes generated.'}</pre></details></motion.div>;
}
...
function ExportView({ exportMarkdown, exportJson, exportDiff, exportPatch, currentFile, copyCode, copied }: { exportMarkdown: () => void; exportJson: () => void; exportDiff: () => void; exportPatch: () => void; currentFile?: ScannedFile; copyCode: () => void; copied: boolean }) {
  ...
  return <motion.div animate={{ opacity: 1, y: 0 }} className="grid gap-2 pt-4 sm:grid-cols-2" initial={reduceMotion ? false : { opacity: 0, y: 14 }} transition={{ duration: 0.5, ease: 'easeOut' }}>{actions.map((item) => { const Icon = item.icon; return <button className="flex items-center justify-between rounded-xl border border-line bg-surface-base/50 p-3.5 text-left text-xs font-extrabold text-ink-primary transition-[transform,border-color,background-color,box-shadow] duration-300 hover:-translate-y-0.5 hover:border-brand-orange/50 hover:bg-brand-orange/5 hover:shadow-panel" key={item.label} onClick={item.action} type="button"><span className="flex items-center gap-2.5"><Icon className="text-brand-orange" size={16} />{item.label}</span><ArrowRight className="text-ink-subtle" size={14} /></button>; })}<div className="sm:col-span-2"><p className="mt-2 flex items-center gap-2 text-xs text-status"><CheckCircle2 size={14} />Report ready for handoff</p><p className="mt-1 text-xs text-ink-muted">{currentFile ? 'Exports contain transformed source and deterministic scan results.' : 'Run a migration to generate exports.'}</p></div></motion.div>;
}
